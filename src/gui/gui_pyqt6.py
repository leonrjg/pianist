...
class PianoFloatingWindow(QWidget):
    def __init__(self):
        ...
        self.fallboard_width = 25
        self.width = 280 - self.fallboard_width  # Start with fallboard hidden (narrower)
        ...
        # Fallboard state (start with fallboard hidden/closed)
        self.fallboard_visible = False
        ...
        # Fallboard slide animation setup
        self.fallboard_animation = QPropertyAnimation(self, b"geometry")
        self.fallboard_animation.setDuration(300)  # 300ms slide duration
        self.fallboard_animation.setEasingCurve(QEasingCurve.Type.InOutQuad)
    ...
    def update_cursor_for_position(self, pos):
        ...
        if self.is_control_button_area(pos) or self.is_key_area(pos) or self.is_brass_section_click(pos) or self.is_brass_hinge_click(pos):
    ...
    def is_key_area(self, pos):
        ...
        # Adjust keys position based on fallboard state
        if self.fallboard_visible:
            keys_start_x = 95
        else:
            keys_start_x = 95 - 25
    ...
    def draw_piano_frame(self, painter):
        ...
        if self.fallboard_visible:
            # Extended: draw full frame + fallboard
            frame_width = 90
            hinge_x = frame_width - fallboard_width
            ...
            # Piano fallboard area (visible when extended)
            fallboard_x = frame_width - fallboard_width
            painter.setBrush(QBrush(self.colors['fallboard']))
            painter.setPen(Qt.PenStyle.NoPen)
            painter.drawRect(fallboard_x, 10, 10 - fallboard_x, self.height - 10 - 10)

            # Key bed at full frame width
            painter.setPen(QPen(QColor(10, 10, 10)))
            painter.setBrush(QBrush(QColor(10, 10, 10)))
            painter.drawRect(frame_width, 0, 5, self.height)

            # Brass hinges at hinge position
            self._draw_brass_elements(painter, [
                (hinge_x + 5, 50 - 4, 8, 8),
                (hinge_x + 5, self.height - 50 - 4, 8, 8)
            ])
        else:
            # Retracted: draw minimal frame, hinges at edge
            frame_width = 0  # No frame visible when retracted
            hinge_x = 0  # Hinges at the edge
    ...
    def draw_piano_keys(self, painter):
        ...
        if self.fallboard_visible:
            ...
            keys_start_x = 95 - 25  # Shift left when fallboard is hidden
        keys_end_x = self.width - 40
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if not self.has_moved:
                ...
                # Check for brass hinge clicks (fallboard toggle)
                if self.is_brass_hinge_click(click_pos):
    ...
    def is_brass_hinge_click(self, click_pos):
        """Check if click is on the brass hinges (fallboard toggle area)"""
        # Hinge position depends on fallboard state
        if self.fallboard_visible:
            hinge_x = 65  # At normal position when extended
        else:
            hinge_x = 0  # At edge when retracted

        hinge_areas = [
            QRect(hinge_x, 50 - 8, 18, 16),  # Top hinge
            QRect(hinge_x, self.height - 50 - 8, 18, 16)  # Bottom hinge
        ]
        for area in hinge_areas:
            if area.contains(click_pos):
                return True
        return False

    def toggle_fallboard(self):
        """Toggle the fallboard visibility with slide animation"""
        self.fallboard_visible = not self.fallboard_visible

        # Calculate new window dimensions
        current_geometry = self.geometry()

        if self.fallboard_visible:
            # Opening: expand window by moving left edge to the left to reveal fallboard
            new_x = current_geometry.x() - self.fallboard_width
            new_width = current_geometry.width() + self.fallboard_width
        else:
            # Closing: shrink window by moving left edge to the right to hide fallboard
            new_x = current_geometry.x() + self.fallboard_width
            new_width = current_geometry.width() - self.fallboard_width

        # Animate the geometry change
        self.fallboard_animation.setStartValue(current_geometry)
        self.fallboard_animation.setEndValue(
            QRect(new_x, current_geometry.y(), new_width, current_geometry.height())
        )
        self.fallboard_animation.start()
