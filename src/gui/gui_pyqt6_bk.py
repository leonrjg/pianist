...
from PyQt6.QtCore import Qt, QTimer, QRect, QPoint, QThread, pyqtSignal, QUrl
...
# Constants
KEY_PRESS_DURATION = 200  # milliseconds
SOUND_FILES = {
...
class PianoFloatingWindow(QWidget):
    ...
    def update_keys_for_window_size(self):
        ...
        if keys_changed:
            ...
            print(f"Updated to {len(self.keys)} keys for window height {self.height}{scroll_status}")
    ...
    def _create_sound_player(self, file_key):
        ...
        if os.path.exists(file_path):
            ...
            print(f"{file_key.capitalize()} sound loaded: {file_path}")
            return player
        return None
    ...
    def draw_piano_frame(self, painter):
        ...
        self.draw_gradient_rect(painter, fallboard_x, 10, frame_width, self.height - 10,
                               self.colors['fallboard'], self.colors['wood_dark'], vertical=True)
        ...
        # Music stand ledge
        painter.setPen(QPen(self.colors['wood_dark']))
        painter.setBrush(QBrush(self.colors['wood_dark']))
        ledge_x = frame_width - 5
        painter.drawRect(ledge_x, 0, 3, self.height)
    ...
    def draw_piano_keys(self, painter):
        ...
        for i, key_data in enumerate(self.keys):
            ...
            if is_pressed:
                ...
                # Add subtle inner shadow effect for pressed state
                shadow_color = QColor(self.colors['white_key_shadow'])
                shadow_color.setAlpha(120)
                painter.setPen(QPen(shadow_color, 2))
                painter.drawRect(keys_start_x + 1, int(y + 3), key_width - 2, int(key_height - 4))
            else:
                # Normal state
        ...
        for i in range(len(self.keys) - 1):
            ...
            y = 10 + ((i + 1) * key_height) - (black_key_height / 2)
    ...
    def draw_piano_details(self, painter):
        ...
        pedal_positions = [self.height // 2 - 40, self.height // 2, self.height // 2 + 40]

        # Draw pedals
        pedal_elements = [(pedal_x - 5, y - 8, 10, 16) for y in pedal_positions]
